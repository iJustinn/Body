...
| --- | --- | --- | --- | --- |
| M1 | Critical | First launch before Health permission | Install fresh build and open Body | App shows health cards with empty states plus the sync control without crashing |
| M2 | Critical | Apple Health authorization | Tap Sync Health Data on a physical device with workout, sleep, heart, and body data | Permission prompt appears; after approval, current month workouts and home health cards render |
| M3 | Critical | Widget reads app snapshot | Sync workouts, add the large widget to Home Screen | Widget calendar matches app snapshot |
...
| M12 | Medium | Appearance theme and accent | In Settings, choose System, Light, and Dark themes; choose several app accents | App color scheme follows the selected theme and app tint/sync button use the selected accent color |
| M13 | High | Home health cards | Sync on a physical device with Apple Health samples | Home shows same-sized passive cards for sleep, resting heart rate, weight, body fat, HRV, blood oxygen, VO2 max, and BMI; cards have no bars, chevrons, or false navigation affordances; Steps are not shown |
