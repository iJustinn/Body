...
- **The watch Sleep page now shows the night's sleep stages.** Between the title and the score, the page draws the iPhone's sleep stages hypnogram straight onto the tinted background with no card: four stage rows (Deep, Core, REM, Awake) with a stage letter on the left, the main session's segments colored per stage with gradient connectors between neighbouring stages, and the night's bed and wake times underneath. The 7-day duration chart shows in that slot only when the snapshot no longer carries the night's stages (after midnight before tonight's sleep syncs, or a night recorded without stages). New `BodyWatch/WatchSleepStagesChartView.swift`.
