...
- **The watch Readiness complication now matches the home hero.** Instead of the ring, the circular and rectangular complications draw the same five bands as the Readiness hero on the watch and iPhone home pages: the current band lights up in its status color, the pill marks where the score sits in that band, and the score reads in the middle. The rectangular complication names the level (Prime, High, Moderate, Low, or Poor) beside the ring instead of repeating the number, inside a thin rounded border line. Both bend the bands into a thicker ring, open at the bottom, that fills a round slot. They use the hero's shared band ranges and pill placement, so they cannot drift apart from it. The corner complication keeps its curved score and band gauge, and a Readiness complication already on a watch face updates in place. On tinted faces the active band and pill take the face tint. Previews for every state are in `watch-screenshots/`. Build remains 2.
...
- **Tidier watch complications.** Every rectangular Body complication now sits inside a thin rounded border line. In the ring complications the reading now comes first with the metric's name under it, both lines the same size, and Training Load names its level (such as Optimal) beside the ring like Readiness does. The ring is larger and sits better centered in the slot, and the number inside every circular ring is a little smaller. A number with three or more digits (100, 1.05, 93.4) steps down a little further so it clears the ring, while two digit numbers keep their size. Build remains 2.
