...
final class HealthMetricDescriptorTests: XCTestCase {
    ...
    func testDescriptorTableMatchesTheShippedWiring() {
        ...
        let expected: [HealthMetricKind: (
            ...
            .bodyMass: (.bodyMass, HKUnit.gramUnit(with: .kilo).unitString, .basics, .latestSample, .daily(.average), nil, false),
            .bodyFatPercentage: (.bodyFatPercentage, HKUnit.percent().unitString, .basics, .latestSample, .daily(.average), nil, true),
            ...
            .bodyMassIndex: (.bodyMassIndex, HKUnit.count().unitString, .basics, .latestSample, .daily(.average), nil, false),
