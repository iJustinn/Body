...
/// Tests that grep Swift source files as a lightweight structural guard
/// (call routing, wiring, exhaustive case lists, invariants) rather than
/// exercising behaviour directly. Split out of `ProjectConfigurationTests`
/// so the non-Swift-source corpus (pbxproj, plists, catalogs, docs) stays
/// separate from Swift-source guards.
final class SourceGuardTests: XCTestCase {
    ...
    func testOnboardingIsWiredIntoLaunchAndSettings() throws {
        ...
        let introView = try BodyTestSupport.sourceText(at: "Body/Views/BodyIntroAnimationView.swift")
    ...
    func testAutoApplyWorkoutEffortSettingIsWiredAndDefaultsOff() throws {
        let keys = try BodyTestSupport.sourceText(at: "BodyMetricsKit/BodyHealthSelections.swift")
    ...
    func testReadinessAICommentSettingIsWiredAndDefaultsOn() throws {
        let keys = try BodyTestSupport.sourceText(at: "BodyMetricsKit/BodyHealthSelections.swift")
        let settings = try BodyTestSupport.sourceText(at: "Body/Views/BodySettingsView.swift")
        let generator = try BodyTestSupport.sourceText(at: "Body/Services/ReadinessCommentGenerator.swift")
        ...
        let readinessModels = try BodyTestSupport.sourceText(at: "BodyMetricsKit/ReadinessModels.swift")
    ...
    func testCustomHealthSourceGroupsAreProGatedOnEveryLayer() throws {
        let storeSource = try BodyTestSupport.sourceText(at: "Body/Services/HealthKitWorkoutStore.swift")
        let engineSource = try healthKitFetchEngineText()
        let resolverSource = try BodyTestSupport.sourceText(at: "BodyWatch/WatchSourceResolver.swift")
        ...
        for caller in ["BodyWatch/WatchDeltaFetcher.swift", "BodyWatch/WatchHealthStore.swift"] {
            let callerSource = try BodyTestSupport.sourceText(at: caller)
    ...
    func testReadinessDailySeriesUsesCachedBaselineContext() throws {
        let source = try BodyTestSupport.sourceText(at: "BodyMetricsKit/ReadinessScoreCalculator.swift")
    ...
    func testAppSheetsShareTheTintedGlassBackdrop() throws {
        let cardBackground = try BodyTestSupport.sourceText(at: "Body/Views/BodyCardBackground.swift")
        ...
        for expectation in expectations {
            let source = try BodyTestSupport.sourceText(at: expectation.file)
            XCTAssertEqual(
    ...
    func testWorkoutShareFlowPresentsFullScreenWithGlassToolbarActions() throws {
        ...
        let workoutsSource = try BodyTestSupport.sourceText(at: "Body/Views/BodyWorkoutsView.swift")
        ...
        let routeHeroSource = try BodyTestSupport.sourceText(at: "Body/Views/Health/BodyWorkoutRouteMapHero.swift")
    ...
    func testWorkoutShareOptionRowsAndPhotoAdjustSteps() throws {
        ...
        let cardSource = try BodyTestSupport.sourceText(at: "Body/Views/Health/BodyWorkoutShareCardView.swift")
        ...
        let cardModelSource = try BodyTestSupport.sourceText(at: "Body/Models/WorkoutShareCard.swift")
    ...
    func testWorkoutShareMonthSummaryContent() throws {
        ...
        // SwiftUI view bodies).
        let monthSummarySource = try BodyTestSupport.sourceText(at: "Body/Models/WorkoutShareMonthSummary.swift")
        ...
        let shareCardSource = try BodyTestSupport.sourceText(at: "Body/Models/WorkoutShareCard.swift")
        ...
        // six-row frame.
        let summaryCardSource = try BodyTestSupport.sourceText(at: "Body/Views/Health/BodyWorkoutShareSummaryCardView.swift")
        ...
        let calendarViewSource = try BodyTestSupport.sourceText(at: "BodyShared/Components/WorkoutCalendarView.swift")
        ...
        let breakdownSource = try BodyTestSupport.sourceText(at: "BodyShared/Components/WorkoutTypeBreakdownView.swift")
        ...
        let summaryCardViewSource = try BodyTestSupport.sourceText(at: "Body/Views/Health/BodyWorkoutShareSummaryCardView.swift")
        ...
        let cardViewSource = try BodyTestSupport.sourceText(at: "Body/Views/Health/BodyWorkoutShareCardView.swift")
        ...
        let localizable = try BodyTestSupport.sourceText(at: "Body/Localizable.xcstrings")
    ...
    func testWorkoutShareLongImageMode() throws {
        ...
        let longSource = try BodyTestSupport.sourceText(at: "Body/Views/Health/BodyWorkoutShareLongCardView.swift")
    ...
    func testWorkoutDetailReservesTheRouteHeroAndDrawsItProgressively() throws {
        ...
        let routeEngineSource = try BodyTestSupport.sourceText(at: "Body/Services/HealthKitFetchEngine+Route.swift")
        ...
        let routeStyleSource = try BodyTestSupport.sourceText(at: "BodyMetricsKit/BodyHealthSelections.swift")
        ...
        let catalog = try BodyTestSupport.sourceText(at: "Body/Localizable.xcstrings")
    ...
    func testMetricDetailFloatsHeroChartCalloutAboveNavigationBar() throws {
        ...
        for file in [
            ...
            let source = try BodyTestSupport.sourceText(at: file)
        ...
        let sourceComparison = try BodyTestSupport.sourceText(at: "Body/Views/Health/Charts/SourceComparisonCharts.swift")
    ...
    func testMetricWarningsAreWiredIntoHomeCardsAndDetail() throws {
        let homeSource = try BodyTestSupport.sourceText(at: "Body/Views/BodyHomeView.swift")
        ...
        let kitSource = try BodyTestSupport.sourceText(at: "BodyMetricsKit/MetricThresholdWarning.swift")
    ...
    func testReadinessDayViewKindStaysInSyncAcrossLists() throws {
        ...
        let preferenceSource = try BodyTestSupport.sourceText(at: "Body/Models/BodyAppearancePreference.swift")
    ...
    func testTrainingLoadDetailShowsIntervalDayBreakdownBelowLineChart() throws {
        ...
        let bandSource = try BodyTestSupport.sourceText(at: "Body/Views/Health/Charts/BandBreakdownChart.swift")
    ...
    func testActivityRingGraphicAnimatesProgressWithCircularSweep() throws {
        let source = try BodyTestSupport.sourceText(at: "Body/Views/BodyActivityRingsDetailView.swift")
    ...
    func testMetricDetailViewGatesProRangesAndDays() throws {
        let source = try BodyTestSupport.sourceText(at: "Body/Views/Health/BodyHealthMetricDetailView.swift")
    ...
    func testLatestReadingQueriesAreBoundedToTheTrendWindow() throws {
        ...
        let watchSource = try BodyTestSupport.sourceText(at: "BodyWatch/WatchDeltaFetcher.swift")
        ...
        let snapshotSource = try BodyTestSupport.sourceText(at: "BodyWatchShared/Models/WatchMetricsSnapshot.swift")
    ...
    func testSourceSelectableBarAndRangeDetailsUsePrimarySecondaryComparisonCharts() throws {
        ...
        let storeSource = try BodyTestSupport.sourceText(at: "Body/Services/HealthKitWorkoutStore.swift")
        let appearanceSource = try BodyTestSupport.sourceText(at: "Body/Models/BodyAppearancePreference.swift")
    ...
    func testSleepStageBreakdownTogglesToOptimalRangeChart() throws {
        let source = try bodyHomeViewText()
        let appearanceSource = try BodyTestSupport.sourceText(at: "BodyMetricsKit/BodyHealthSelections.swift")
    ...
    func testAppEntryHydratesDaySamplesBeforeSync() throws {
        let appSource = try BodyTestSupport.sourceText(at: "Body/BodyApp.swift")
    ...
    func testBasicsChartsMorphAcrossRangeSwitchesLikeTheOtherTrendCharts() throws {
        let source = try BodyTestSupport.sourceText(at: "Body/Views/Health/Charts/BasicsCharts.swift")
    ...
    func testChartLegendNumbersFlipWhenTheirValuesChange() throws {
        let helpers = try BodyTestSupport.sourceText(at: "Body/Views/Health/ChartHelpers.swift")
    ...
    func testEffortFetchCannotResurrectAClearedLedger() throws {
        let engineSource = try BodyTestSupport.sourceText(at: "Body/Services/HealthKitFetchEngine.swift")
    ...
    func testTrackedQueryWrappersReleaseTheirPermitOnCancellation() throws {
        let engineSource = try BodyTestSupport.sourceText(at: "Body/Services/HealthKitFetchEngine.swift")
        let budgetSource = try BodyTestSupport.sourceText(at: "Body/Services/HealthKitQueryBudget.swift")
    ...
    func testSecondarySleepStageHistorySkipsVitalsHydration() throws {
        let sleepSource = try BodyTestSupport.sourceText(at: "Body/Services/HealthKitFetchEngine+Sleep.swift")
        let secondarySource = try BodyTestSupport.sourceText(at: "Body/Services/HealthKitFetchEngine+Secondary.swift")
    ...
    func testWorkoutsPageShowsOneChartAtATimeWithAPersistedSwitch() throws {
        ...
        let appearanceSource = try BodyTestSupport.sourceText(at: "BodyMetricsKit/BodyHealthSelections.swift")
        let calendarSource = try BodyTestSupport.sourceText(at: "BodyShared/Components/WorkoutCalendarView.swift")
        let breakdownSource = try BodyTestSupport.sourceText(at: "BodyShared/Components/WorkoutTypeBreakdownView.swift")
        let widgetSource = try BodyTestSupport.sourceText(at: "BodyWidgetExtension/WorkoutCalendarWidget.swift")
    ...
    func testSummaryTabUsesHealthDashboardIcon() throws {
        let source = try BodyTestSupport.sourceText(at: "Body/Views/MainTabView.swift")
    ...
    func testVersionDocumentationAndSettingsFallbackMatchCurrentRelease() throws {
        ...
        XCTAssertTrue(readme.contains("Current app version: **1.1.0 (build 2)**"))
        ...
        XCTAssertTrue(versionHistory.contains("Updated the app, widget, watch, and test bundle version to 1.1.0 build 2."))
    ...
    func testWorkoutRouteReadTypeIsRequested() throws {
        let readTypes = try BodyTestSupport.sourceText(at: "BodyWatchSnapshotKit/BodyHealthReadTypes.swift")
    ...
    func testHealthKitPermissionSheetOnlyOnUserInitiatedEntryPoints() throws {
        ...
        guard let writeSource = try? BodyTestSupport.sourceText(at: "Body/Services/HealthKitFetchEngine+Write.swift") else {
    ...
    func testReadinessImpactCardExplainsItselfInASheet() throws {
        let detailSource = try BodyTestSupport.sourceText(at: "Body/Views/Health/BodyHealthMetricDetailView.swift")
        let sheetSource = try BodyTestSupport.sourceText(at: "Body/Views/Health/BodyReadinessImpactExplanationSheet.swift")
    ...
    func testWorkoutDetailsCardExplainsItsMetricsInASheet() throws {
        let workoutsSource = try BodyTestSupport.sourceText(at: "Body/Views/BodyWorkoutsView.swift")
        let sheetSource = try BodyTestSupport.sourceText(at: "Body/Views/BodyWorkoutDetailsExplanationSheet.swift")
    ...
    func testWorkoutListSheetRowsOpenFullScreenDetail() throws {
        let source = try BodyTestSupport.sourceText(at: "Body/Views/BodyWorkoutListSheet.swift")
    ...
    func testWorkoutCustomNamePlumbing() throws {
        ...
        let listSheetSource = try BodyTestSupport.sourceText(at: "Body/Views/BodyWorkoutListSheet.swift")
        ...
        let summarySource = try BodyTestSupport.sourceText(at: "BodyMetricsKit/WorkoutSummary.swift")
    ...
    func testWorkoutHeartRateSeriesReadFailureIsNotCachedAsEmpty() throws {
        ...
        let seriesSource = try BodyTestSupport.sourceText(at: "Body/Services/HealthKitFetchEngine+MetricSeries.swift")
    ...
    func testWorkoutStepSamplesPropagateReadFailuresInsteadOfSwallowingErrors() throws {
        let splitsSource = try BodyTestSupport.sourceText(at: "Body/Services/HealthKitFetchEngine+Splits.swift")
    ...
    func testBackgroundWarningRefreshIsRegisteredAndScoped() throws {
        ...
        let schedulerSource = try BodyTestSupport.sourceText(at: "Body/Services/BodyBackgroundRefreshScheduler.swift")
        ...
        let evaluatorSource = try BodyTestSupport.sourceText(at: "Body/Services/MetricWarningBackgroundEvaluator.swift")
    ...
    func testHealthSyncBadgeIsWiredIntoMainTabWithGlassAndMaterialFallback() throws {
        let mainTabSource = try BodyTestSupport.sourceText(at: "Body/Views/MainTabView.swift")
        let badgeSource = try BodyTestSupport.sourceText(at: "Body/Views/BodyHealthSyncBadge.swift")
        let homeSource = try BodyTestSupport.sourceText(at: "Body/Views/BodyHomeView.swift")
        let metricDetailSource = try BodyTestSupport.sourceText(at: "Body/Views/Health/BodyHealthMetricDetailView.swift")
        let workoutsSource = try BodyTestSupport.sourceText(at: "Body/Views/BodyWorkoutsView.swift")
        let storeSource = try BodyTestSupport.sourceText(at: "Body/Services/HealthKitWorkoutStore.swift")
    ...
    func testDeadChartsViewAndHealthCardAccessoryBranchAreRemoved() throws {
        let oldChartsViewURL = BodyTestSupport.projectRoot.appendingPathComponent("Body/Views/BodyChartsView.swift")
        let chartsSource = try BodyTestSupport.sourceText(at: "Body/Views/BodyWorkoutListSheet.swift")
        let homeSource = try bodyHomeViewText()
    ...
    func testProjectDateMathUsesBodyGregorianForSleepAxisAndWidgetTimeline() throws {
        ...
        let calendarBuilderSource = try BodyTestSupport.sourceText(at: "BodyShared/Widgets/WorkoutCalendarEntryBuilder.swift")
        XCTAssertTrue(calendarBuilderSource.contains("calendar.date(byAdding: .minute"))
    }
    ...
    func testSettingsUnitsPageHasSystemToggleAndIndependentUnitControls() throws {
        let settingsSource = try BodyTestSupport.sourceText(at: "Body/Views/BodySettingsView.swift")
        let appearanceSource = try BodyTestSupport.sourceText(at: "BodyMetricsKit/BodyHealthSelections.swift")
        let formatterSource = try BodyTestSupport.sourceText(at: "BodyMetricsKit/WorkoutSummary.swift")
    ...
    func testBodyProStoreRevenueCatWiringIsGuarded() throws {
        let storeSource = try BodyTestSupport.sourceText(at: "Body/Services/BodyProStore.swift")
        let clientSource = try BodyTestSupport.sourceText(at: "Body/Services/RevenueCatPurchasesClient.swift")
        let configSource = try BodyTestSupport.sourceText(at: "Body/Services/RevenueCatConfiguration.swift")
        let entitlementSource = try BodyTestSupport.sourceText(at: "BodyShared/Services/BodyProEntitlement.swift")
        let widgetSource = try BodyTestSupport.sourceText(at: "BodyWidgetExtension/HealthMetricWidget.swift")

        // Backed by RevenueCat, keyed to the single non-consumable that unlocks Pro. The
        // store owns the state machine and stays SDK-free: every RevenueCat call lives
        // behind `BodyPurchasesClient`, which is what makes the flow testable.
        XCTAssertFalse(storeSource.contains("import RevenueCat"))
        ...
        XCTAssertTrue(clientSource.contains("Purchases.shared.customerInfoStream"))
        XCTAssertTrue(clientSource.contains("Purchases.shared.purchase(product:"))
        XCTAssertTrue(clientSource.contains("Purchases.shared.restorePurchases()"))
        XCTAssertTrue(clientSource.contains("entitlements[RevenueCatConfiguration.proEntitlementID]?.isActive == true"))
        ...
        XCTAssertTrue(clientSource.contains("customerInfo(fetchPolicy: .fetchCurrent)"))
        ...
        XCTAssertTrue(clientSource.contains(".paymentPendingError"))
        ...
        // back to the buy card: the client does one bounded network re-check (the second
        ...
        let proViewSource = try BodyTestSupport.sourceText(at: "Body/Views/BodyProView.swift")
        ...
        for source in [storeSource, clientSource] {
            XCTAssertFalse(source.contains("Transaction.updates"))
            XCTAssertFalse(source.contains("Transaction.currentEntitlements"))
            XCTAssertFalse(source.contains("transaction.finish()"))
            XCTAssertFalse(source.contains("AppStore.sync()"))
            XCTAssertFalse(source.contains("purchasesAreCompletedBy: .myApp"))
            XCTAssertFalse(source.contains("recordPurchase"))
        }
    ...
    func testExerciseWeekWidgetsArePinnedToAccessoryRectangular() throws {
        ...
        let phoneBundle = try BodyTestSupport.sourceText(at: "BodyWidgetExtension/BodyWidgetExtensionBundle.swift")
        ...
        let complicationTimelineSource = try BodyTestSupport.sourceText(at: "BodyWatchShared/Services/WatchComplicationTimeline.swift")
        XCTAssertTrue(watchSource.contains("WatchComplicationTimeline.exerciseWeekMetric(in: entry.snapshot)"))
        XCTAssertTrue(complicationTimelineSource.contains("snapshot.metric(forKind: WatchMetricKindKey.workoutMinutes)"))
    ...
    func testSleepStagesComplicationIsPinnedToAccessoryRectangularAndMirrorsStageColors() throws {
        ...
        let watchBundle = try BodyTestSupport.sourceText(at: "BodyWatchWidgetExtension/BodyWatchComplicationsBundle.swift")
        let healthWidgetSnapshotSource = try BodyTestSupport.sourceText(at: "BodyShared/Models/HealthWidgetSnapshot.swift")
