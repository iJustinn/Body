...
struct BodyDayRingHero: View {
        ...
        TimelineView(.everyMinute) { context in
            ...
            ZStack(alignment: .topLeading) {
                ...
                centerText(percent: percent)
                    .opacity(BodyReadinessArcGeometry.textOpacity(progress: min(max(progress, 0), 1), width: width))
            }
            .offset(y: -pull)
    ...
    // MARK: - Center text (the Readiness Ring's score block, without badges)
    ...
    private var numberCenterY: CGFloat {
        ...
        return showsCaption ? centered - (Self.captionHeight + Self.captionGap) / 2 : centered
    }
